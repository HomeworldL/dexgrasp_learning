...
class FlowTargetCodec:
    """结构化抓取字段与 flow 目标向量之间的编解码器。"""
    ...
    def __init__(
        ...
        if pose_scale <= 0.0:
            raise ValueError(
                f"target_normalization.pose_scale must be positive, got {pose_scale}."
            )
        if joint_scale <= 0.0:
            raise ValueError(
