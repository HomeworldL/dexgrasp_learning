...
from models.dexdiffuser.utils import build_mlp


class DexDiffuserRTHead(nn.Module):
    ...
    def forward(
        ...
        if global_feature.ndim != 2:
            raise ValueError(
                "DexDiffuserRTHead expects global_feature with shape [B, C], "
        ...
        if squeeze_pose.ndim != 3:
            raise ValueError(
                "DexDiffuserRTHead expects squeeze_pose with shape [B, D] or [B, K, D], "
