...
from models.backbones.pointnet2.pointnet2_modules import (
    PointnetFPModule,
    PointnetSAModule,
    PointnetSAModuleMSG,
)
import models.backbones.pointnet2.pytorch_utils as pt_utils


DEFAULT_MSG_NPOINTS = [1024, 256, 64, 16]
DEFAULT_MSG_RADIUS = [[0.05, 0.1], [0.1, 0.2], [0.2, 0.4], [0.4, 0.8]]
DEFAULT_MSG_NSAMPLE = [[16, 32], [16, 32], [16, 32], [16, 32]]
DEFAULT_MSG_MLPS = [
    [[16, 16, 32], [32, 32, 64]],
    [[64, 64, 128], [64, 96, 128]],
...
def _resolve_msg_config(npoints=None, radii=None, nsamples=None, mlps=None):
    return (
        list(npoints) if npoints is not None else deepcopy(DEFAULT_MSG_NPOINTS),
        deepcopy(radii) if radii is not None else deepcopy(DEFAULT_MSG_RADIUS),
        deepcopy(nsamples) if nsamples is not None else deepcopy(DEFAULT_MSG_NSAMPLE),
        deepcopy(mlps) if mlps is not None else deepcopy(DEFAULT_MSG_MLPS),
    )
...
class Pointnet2MSG_Feature(nn.Module):
    def __init__(
        ...
        NPOINTS, RADIUS, NSAMPLE, MLPS = _resolve_msg_config(
            npoints=npoints,
            radii=radii,
            nsamples=nsamples,
            mlps=mlps,
