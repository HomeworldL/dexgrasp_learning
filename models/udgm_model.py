...
class UDGMModel(BaseModel):
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        flow_target_dim = self.target_dim
        ...
        if self.prediction_structure_name == "flat":
            ...
            flow_target_dim = self.squeeze_pose_dim
        else:
            raise NotImplementedError(
                "UDGMModel only supports model.prediction_structure.name=flat or staged."
            )
        self.require_algorithm_config("condition_dim")
        self.require_algorithm_config("flow.hidden_dim")
        self.require_algorithm_config("flow.num_layers")
        self.require_algorithm_config("flow.num_blocks_per_layer")

        self.flow = UDGMFlow(
            target_dim=flow_target_dim,
            condition_dim=self.condition_dim,
        ...
        if self.loss_clamp_max is not None:
            ...
            if self.loss_clamp_max <= 0.0:
                raise ValueError(
                    f"model.algorithms.udgm.{self.prediction_structure_name}.flow.loss_clamp_max "
                    f"must be positive, got {self.loss_clamp_max}."
                )
        ...
        if self.prediction_structure_name == "staged":
            ...
            if self.training_pose_num_samples <= 0:
                raise ValueError(
                    "model.algorithms.udgm.staged.flow.training_pose_num_samples "
                    f"must be positive, got {self.training_pose_num_samples}."
                )
            regression_config = self.get_staged_regression_config()
            self.regression_head = UDGMStagedHead(
                condition_dim=self.condition_dim,
                squeeze_pose_dim=self.squeeze_pose_dim,
                init_pose_dim=self.init_pose_dim,
                joint_dim=self.joint_dim,
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        sampled_squeeze_pose = self.squeeze_pose_codec.split(sampled_squeeze_pose)[
            "pred_squeeze_pose"
        ][:, 0, :]
        ...
        if self.loss_weights is None:
            raise RuntimeError("Staged UDGM loss weights were not initialized.")
        loss = (
